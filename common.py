...
class GridPlot:
    ...
    def new_figure(self, name, key):
        ...
        self.figs[key].begin(max_cols=self.max_cols, title=name + ' ' + key, size=self.figsize, id=key)
    ...
    def end_draw(self):
        if self.glyph_data:
            ...
            sbox = geom.bounding_box(self.S, 30)
            ...
            box = geom.bounding_box(self.S, 30)
        plut.setup(axis=False, ax=self.ax, axis_limits=box) #geom.bounding_box(self.S, 30), ax=self.ax)
        self.ax = None
                ...
                for key, mfig in list(self.figs.items()):
                    if cur_name:
                        if kind != 'ttf' and count >= max_count:
                            ...
                            if save_path:
                                mfig.end(os.path.join(save_path, cur_name + '_' + key + '.pdf'))
                            else:
                                mfig.end() 
                    if not bail:
                        self.figs[key] = plut.MultiFigure()
...
def load_glyph(db, font_index, char, size=150, resample=True):
    try:   
        ...
        #pdb.set_trace()
        S, ratio = geom.rescale_and_sample_vertical(S, closed=True, height=glyph_h, dest_height=size, get_ratio=True)
...
def rasterize_shape(shape, raster_size, src_rect=None, get_im_and_mat=True, im_mat=None, padding=3, outline_shape=[]):
    ...
    if im_mat is None:
        ...
        im_mat = geom.rect_in_rect_transform(src_rect, dst_rect, padding=padding)
    #endif
...
def load_svg(path, size, flip_y=False, union=True, closed=True, get_data=False):
    ...
    S, ratio = geom.rescale_and_sample_vertical(S, closed=closed, height=0, dest_height=size, get_ratio=True)
    if flip_y:
...
def preprocess_shape(S, size, get_data=False):
    import autograff.svg as svg
    if type(S)!=list:
        S = [S]
        is_contour = True
    ...
    S, ratio = geom.rescale_and_sample_vertical(S, closed=True, height=0, dest_height=size, get_ratio=True)
    S = [P/ratio for P in S]
    S = geom.fix_shape_winding(S)    
    
    glyph_data = {'height': size/ratio, 'width': size, 'ratio': ratio, 'target_size': size}
    if is_contour:
        S = S[0]
    ...
    def iterate(self, size=100, paths=[], union=False):
        for path in self.paths:
            if paths and utils.filename_without_ext(path) not in paths:
                ...
                continue
            shape, glyph_data = load_svg(path, size=size, flip_y=self.flip_y, union=union, get_data=True)
            #pdb.set_trace()
...
def load_shape_structure(path, size, flip_y):
    ...
    with open(path) as json_file:
        ...
        for p in points:
            P.append([p['x'], p['y']])
        P = np.array(P).T
        S, ratio = geom.rescale_and_sample([P], closed=True, scale=size, get_ratio=True)
        if flip_y:
            for i, P in enumerate(S):
                S[i][1,:] = -P[1,:]
...
class FontDataSource:
    ...
    def __init__(self, path, prefix):
        ...
        self.paths = {utils.filename_without_ext(f):f for f in utils.files_in_dir(path) if '.pkl' in f}
        
    def size(self):
        return len(self.paths)

    def make_name(self, name, char, forcelower=False):
        if char.islower() and not forcelower: # so it seems that python/osx does not overwrite case sensitive. but loads case sensitive??
