...
def compute_casa_null(MA, features, sign=1, include_flexures=True, include_terminals=True):
    ...
    MA_ext.graph['tip_features'] = {}
    MA_ext.graph['flexure_convexities'] = set()
...
def compute_casa(MA, features, sign=1, include_flexures=True, include_terminals=True):
    ...
    MA_ext.graph['MA_terminals'] = terminals
    return MA_ext
...
def is_casa_only_fork(MA, v):
    """ Returns true if v is a fork and it is in CASA but not in MA"""
    MA_forks = MA.graph['MA_forks']
    if MA.degree(v) < 3:
        return False
    return v not in MA_forks

def is_casa_only_terminal(MA, v):
    """ Returns true if v is a terminal and it is in CASA but not in MA"""
    if MA.degree(v) != 1:
        return False
    return v not in MA.graph['MA_terminals']
